...
import 'package:tournament_app/core/router/app_router.dart';
import '../../../../core/theme/app_colors.dart';
import '../../../../core/theme/app_text_styles.dart';
import '../../../common/widgets/shared_widgets.dart';
import '../../../../domain/entities/leaderboard_entity.dart';
import '../screens/leaderboard_notifier.dart';
import '../../tournament/detail/tournament_detail_notifier.dart';
...
class _LeaderboardScreenState extends ConsumerState<LeaderboardScreen>
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(leaderboardNotifierProvider(widget.tournamentId));

    return Scaffold(
      backgroundColor: AppColors.bg,
      appBar: AppBar(
        automaticallyImplyLeading: false,
        title: Text('Leaderboard', style: AppTextStyles.heading(size: 18)),
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh, color: AppColors.muted),
            tooltip: 'Refresh',
            onPressed: () => ref
                .read(leaderboardNotifierProvider(widget.tournamentId).notifier)
                .refresh(),
          ),
          IconButton(
            icon: const Icon(Icons.share_outlined),
            onPressed: () {
              final tournamentDetail = ref.read(
                tournamentDetailNotifierProvider(widget.tournamentId),
              );
              final tournamentName =
                  tournamentDetail.tournament?.name ?? 'Tournament';
              final organizerTag =
                  tournamentDetail.tournament?.organizerTag ?? '';

              // Compute total distinct match numbers from breakdowns
              final allMatchNums = <int>{};
              for (final m in state.matchBreakdowns.values) {
                allMatchNums.addAll(m.keys);
              }
              final totalMatches =
                  allMatchNums.isEmpty ? 1 : allMatchNums.length;

              context.push(
                AppRoutes.leaderboardExport,
                extra: {
                  'tournamentName': tournamentName,
                  'organizerTag': organizerTag,
        ...
        child: Container(
          ...
          child: state.isLoading
              ...
              : state.errorMessage != null
                  ? EmptyState(title: 'Error', subtitle: state.errorMessage!)
                  : TabBarView(
                      controller: _tab,
                      children: [
                        // ── Point Table Tab ──────────────────────
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              ...
              child: Text(
                'Total',
                style: AppTextStyles.label(color: AppColors.white, size: 10),
                textAlign: TextAlign.center,
              ),
            ),
          ],
...
class _PointTableRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          color: position <= 3 ? _posColor.withOpacity(0.06) : AppColors.bg3,
          borderRadius: BorderRadius.circular(10),
          border: Border.all(
            color:
                position <= 3 ? _posColor.withOpacity(0.3) : AppColors.border,
          ),
        ),
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                entry.teamName,
                style:
                    AppTextStyles.subheading(color: AppColors.white, size: 12),
...
class _MvpList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final reversed = mvpList.reversed.toList();
    return ListView.separated(
      padding: const EdgeInsets.all(16),
      itemCount: reversed.length,
      separatorBuilder: (_, __) => const SizedBox(height: 8),
      itemBuilder: (_, i) => _MvpCard(mvp: reversed[i]),
    );
  }
}
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: _rankColor(mvp.mvpRank).withOpacity(0.5)),
              ),
